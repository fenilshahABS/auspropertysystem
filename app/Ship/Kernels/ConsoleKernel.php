...
class ConsoleKernel extends LaravelConsoleKernel
{
    ...
    {
        // $schedule->command('inspire')->hourly();RentInvoiceGeneration
        $schedule->command('apiato:TaskRemainder')->everyFiveMinutes();
        $schedule->command('apiato:MovePropertyToRent')->everyMinute();

        $schedule->command('apiato:RentInvoiceGeneration')->everyMinute();
        $schedule->command('apiato:SendRentInvoiceOnMail')->everyFiveMinutes();
        $schedule->command('apiato:OwnerShareInvoiceGeneration')->everyFiveMinutes();
    }
