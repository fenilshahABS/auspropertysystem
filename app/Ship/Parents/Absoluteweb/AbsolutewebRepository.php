    ...
    */
    ...
    /*
      ...
      */
      ...
      public static function sendPropertyNotification($getData)
      {

          $userData =  Tenantusers::where('role_id',1)->get();
          if(!empty($userData)){
              for($k=0;$k<count($userData);$k++){
                  $user_to_notify = $userData[$k]->id;
                  $user_who_fired_event = Auth::id() ?? $getData->id;
                  $message = 'New Property Added..Please review.';
                  $sendNotification = Notifications::create([
                      'user_to_notify' => $user_to_notify, // Admin
                      'user_who_fired_event' => $user_who_fired_event, // Auth
                      'message' => $message,
                      'is_seen' => 0,
