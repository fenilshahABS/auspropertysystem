...
class GetAllRentalInvoiceManualsTask extends ParentTask
{
    use HashIdTrait;
    ...
    public function run($InputData, $request)
    {
        try {
            ...
            if (!empty($user_id)) {
                ...
                $query->leftjoin('pro_property_master_share_details', 'pro_property_master_share_details.pro_property_master_id', 'pro_rentals_invoice_manual.property_id');
            ...
            if (!empty($property_master_id)) {
                $query->where('pro_rentals_invoice_manual.property_id', $property_master_id);
            }
            ...
            if ($request->flag == 'reports') {
                $getData = $query->orderBy('pro_rentals_invoice_manual.id', 'DESC')->get();
            } else {
            ...
            if (!empty($getData) && count($getData) >= 1) {
                ...
                for ($i = 0; $i < count($getData); $i++) {
                    ...
                    $rentalInvoiceManualDetails = RentalInvoiceManualDetails::where('rent_invoice_id', $getData[$i]['id'])->get();
                    $returnDetails = array();
                    ...
                    $rental_invoice_transactions = RentalInvoiceTransactionsManual::where('rental_invoice_id', $getData[$i]['id'])->orderBy('id', 'DESC')->get();
