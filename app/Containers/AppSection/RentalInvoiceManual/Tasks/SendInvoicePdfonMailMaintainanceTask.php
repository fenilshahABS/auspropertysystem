...
class SendInvoicePdfonMailMaintainanceTask extends ParentTask
{
    use HashIdTrait;
    ...
    public function run($path, $InputData)
    {
        try {
            ...
            $getRentalPropertyManagement = RentalPropertyManagement::select('property_master_id')->where('id',$property_id)->first();
            if(!empty($getRentalPropertyManagement)){
              $property_id = $getRentalPropertyManagement->property_master_id;
