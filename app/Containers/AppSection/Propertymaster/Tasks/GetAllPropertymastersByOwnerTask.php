...
class GetAllPropertymastersByOwnerTask extends ParentTask
{
    use HashIdTrait;
    ...
    public function run($InputData)
    {
        try {
            ...
            if(empty($user_id)){
              ...
              $query = Propertymaster::select('pro_property_master.*');
              if (!empty($user_id)) {
                  $query->leftjoin('pro_property_master_share_details', 'pro_property_master_share_details.pro_property_master_id', 'pro_property_master.id');
                  $query->where('pro_property_master_share_details.property_owner_id', $user_id);
              }
              $getData = $query->orderBy('pro_property_master.created_at', 'DESC')
                               ->get();
                ...
                for ($i = 0; $i < count($getData); $i++) {
                    ...
                    if (!empty($propertyshare_details) && count($propertyshare_details)) {
                        for ($k = 0; $k < count($propertyshare_details); $k++) {
                            ...
                            $owner_details = Tenantusers::where('id',$propertyshare_details[$k]->property_owner_id)->first();
                            if(!empty($owner_details)){
                              $returnShareDetails[$k]['property_owner_name'] = $owner_details->first_name." ".$owner_details->last_name;
                              $returnShareDetails[$k]['property_owner_email'] = $owner_details->email;
                              $returnShareDetails[$k]['property_owner_mobile'] = $owner_details->mobile;
                            }else{
                              $returnShareDetails[$k]['property_owner_name'] = "";
                              $returnShareDetails[$k]['property_owner_email'] = "";
                              $returnShareDetails[$k]['property_owner_mobile'] = "";
                            }
