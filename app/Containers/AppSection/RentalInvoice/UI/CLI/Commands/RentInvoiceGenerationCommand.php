                ...
                if ($currentDate >= $lease_start_date  && $currentDate <= $lease_end_date) {
                    ...
                    if (empty($checkExistEntry)) {
                        ...
                        if ($getRentData[$i]->rent_frequency == "monthly") {
                            ...
                            //if ($interval->days >= 30) {
                                $invoiceData = [
                                    ...
                                    "invoice_date_gen" => $getRentData[$i]->lease_start_date,
                      ...
                      // every Time
                        ...
                        Log::info("Rent Invoice id ".$checkExistEntry->id);
                        //Log::info("Rent Invoice status ".$checkExistEntry->status);

                            if ($getRentData[$i]->rent_frequency == "monthly") {
                                //if ($dayInterval >= 30) {
                              $invoice_date_gen_new = Carbon::parse($invoice_date_gen)->addMonth()->format('Y-m-d');
                              Log::info("Rent invoice_date_gen_new ".$invoice_date_gen_new);
                              Log::info("Rent invoice_date_gen ".$checkExistEntry->invoice_date_gen);
                              if($invoice_date_gen_new >= $checkExistEntry->invoice_date_gen){
                                Log::info("Rent currentDate ".$date);
                                if($date >= $invoice_date_gen_new){
                              ...
                              //$dayInterval = $dayInterval->days;
                                    ...
                                    $invoiceData = [
                                        ...
                                        "invoice_type" => "RN",
                                        "invoice_date_gen" => $invoice_date_gen_new,
                                        "amount_total" => $getRentData[$i]->rent_amount,
                                        "pending_amount" => $getRentData[$i]->rent_amount,
                                        "status" => 0,
                                    ];
                              ...
                              //}
                                ...
                                //if ($dayInterval >= 365) {
                                $invoice_date_gen_new = Carbon::parse($invoice_date_gen)->addYear()->format('Y-m-d');
                                Log::info("Rent invoice_date_gen_new ".$invoice_date_gen_new);
                                Log::info("Rent invoice_date_gen ".$checkExistEntry->invoice_date_gen);
                                if($invoice_date_gen_new >= $checkExistEntry->invoice_date_gen){
                                  Log::info("Rent currentDate ".$date);
